...
void OpenGLBuffers::sendDataToGPU(Vertex * data, int numVertices) {
	...
	/*glPointSize(2.0f);
	if (numVertices == 1)glDrawArrays(GL_POINTS, 0, numVertices);
	else {
		glPointSize(5.0f);
		for (int i = 0; i < numVertices; i++) glDrawArrays(GL_POINTS, i, numVertices);
	}*/
...
void OpenGLBuffers::sendDataToGPUQuad(Vertex * data, int numVertex)
{
}

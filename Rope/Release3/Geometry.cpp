...
void Geometry::loadGameElementFromASE(char fileName[100]) {
	_objectLoader.loadAse(fileName, _numVertices, _verticesData);
}
...
void Geometry::loadScene(string fileName)
{
	...
	go._objectType = CUBE;
	go._translate = glm::vec3(-0.3f, 2.3f, 0.0f);
	go._angle = 0.0f;
	go._rotation = glm::vec3(0.0f, 0.0f, 0.0f);
	go._scale = glm::vec3(1.0f, 1.0f, 1.0f);
	go._mode = 2;
	...
	if (file.is_open()) {
		file >> maxObjects;
		file >> maxParticles;
		_listOfObjects.reserve(maxObjects + maxParticles + 1);
		if (_listOfObjects.size() == 0) {
			for (int i = 0; i < maxObjects; i++) {
				GameObject newObject;
				file >> newObject._objectType;
				file >> newObject._translate.x;
				file >> newObject._translate.y;
				file >> newObject._translate.z;
